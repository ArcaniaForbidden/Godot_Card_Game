...
func update_battle_zones() -> void:
	...
	for battle in active_battles.duplicate():
		# Add any new cards that enter this battle zone
		check_and_add_cards(battle)
		# Recalculate the bounding rect for all cards in this zone
		battle.rect = calculate_battle_zone_rect(battle)
		if battle.visual:
			battle.visual.position = battle.rect.position
			battle.visual.size = battle.rect.size
		# Merge any overlapping battles
		merge_overlapping_battles()
		# Lock cards inside the battle zone (positions & z_index)
		lock_cards_in_zone(battle)
		# End the battle if no units or enemies remain
		if battle.units.is_empty() or battle.enemies.is_empty():
			end_battle(battle)

func merge_overlapping_battles() -> void:
	# Use a copy of active_battles to avoid modifying the array while iterating
		...
		for j in range(i + 1, active_battles.size()):
			...
			if battle_a.rect.intersects(battle_b.rect):
				...
				for unit_card in battle_b.units:
					...
					if unit_card not in battle_a.attack_timers:
						battle_a.attack_timers[unit_card] = battle_b.attack_timers.get(unit_card, 0.0)
					mark_card_in_battle(unit_card)
				# --- Merge enemies ---
				for enemy_card in battle_b.enemies:
					if enemy_card not in battle_a.enemies:
						battle_a.enemies.append(enemy_card)
					if enemy_card not in battle_a.attack_timers:
						battle_a.attack_timers[enemy_card] = battle_b.attack_timers.get(enemy_card, 0.0)
					mark_card_in_battle(enemy_card)
				# --- Merge battle centers and recalc rect ---
				battle_a.center_position = (battle_a.center_position + battle_b.center_position) * 0.5
				# Recalculate bounding rect for merged cards
				...
				if battle_b.visual:
					...
					battle_b.visual = null
				# --- Remove old battle ---
				active_battles.erase(battle_b)
				# --- No recursion needed, recheck from scratch next frame ---
				return
...
func check_and_add_cards(battle: BattleZone) -> void:
	...
	for stack in card_manager.all_stacks:
		...
		for card in stack:
			if card.card_type in ["unit", "building"] and not card.in_battle:
				unit_cards.append(card)
...
func process_battle_combat(delta: float) -> void:
	for battle in active_battles.duplicate():
		# Make a copy of all cards to safely iterate
		var all_units = (battle.units + battle.enemies).duplicate()
		for card in all_units:
			# Skip invalid cards or ones no longer tracked
			if not is_instance_valid(card) or not battle.attack_timers.has(card):
				continue
			# Update attack timer
			...
			if battle.attack_timers[card] >= attack_interval:
				...
				if target_card.health <= 0:
					...
					if stack.size() > 0:
						...
						if stack.is_empty():
							card_manager.all_stacks.erase(stack)
					# Remove attack timer to avoid dictionary issues
