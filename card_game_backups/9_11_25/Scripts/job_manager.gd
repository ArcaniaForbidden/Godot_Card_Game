...
func check_all_stacks() -> void:
	...
	for stack in card_manager.all_stacks:
		...
		for recipe in RecipeDatabase.recipes:
			if is_stack_matching_recipe(stack, recipe):
...
func update_jobs(delta: float) -> void:
	for job in active_jobs.duplicate():
		...
		if not stack_still_registered:
			print("⏹ Job canceled (tracked stack removed). Recipe:", recipe["name"], "Tracked stack:", stack_to_names(stack))
		...
		if should_cancel:
			...
			print("⏹ Job canceled (card removed or entered battle). Stack:", names, "Recipe:", recipe["name"])
			remove_progress_bar(job)
			active_jobs.erase(job)
			continue
		...
		if not is_stack_matching_recipe(stack, recipe):
			...
			print("⏹ Job canceled (stack no longer matches). Stack:", names2, "Recipe:", recipe["name"])
			remove_progress_bar(job)
			active_jobs.erase(job)
			continue
		...
		if job["__progress_print_timer"] >= 0.5:
			...
			print("Job progress:", str(snapped(job["progress"], 0.01)), "/", job["work_time"], "Stack:", progress_names, "Recipe:", recipe["name"])
		...
		if job["progress"] >= job["work_time"]:
			...
			print("✅ Job complete! Recipe:", recipe["name"], "Stack:", progress_names2)
			...
			if "outputs" in recipe:
				for output_dict in recipe["outputs"]:
					...
					for t_i in range(amount):
						var spawn_pos = stack[0].global_position if stack.size() > 0 else Vector2.ZERO
						var new_card = card_manager.spawn_card(subtype, spawn_pos)
						# Append to stack (not to the dragged_substack)
						stack.append(new_card)
						print("Produced output:", new_card.subtype, "on stack")
...
func start_job(stack: Array, recipe: Dictionary) -> void:
	...
	print("▶ Job started. Stack:", stack_to_names(stack), "Recipe:", recipe["name"])

func cancel_job(job: Dictionary) -> void:
	print("⏹ Job canceled. Stack:", stack_to_names(job["stack"]), "Recipe:", job["recipe"]["name"])
	active_jobs.erase(job)
