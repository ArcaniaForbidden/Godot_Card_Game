...
var enemy_idle_timer: float = 0.0  		# time until next jump
...
@onready var label_nodes: Dictionary[String, Label] = {
	"health": get_node_or_null("HealthLabel"),
	"attack": get_node_or_null("AttackLabel"),
	"armor": get_node_or_null("ArmorLabel")
}
...
func setup(subtype_name: String) -> void:
	...
	for stat_name in ["health", "attack", "armor"]:
		if data.has(stat_name):
			...
			if stat_name == "health":
				max_health = int(data[stat_name])
		else:
			# Explicitly hide icon and clear label if stat not present
			if icon_nodes.has(stat_name) and icon_nodes[stat_name]:
				icon_nodes[stat_name].visible = false
			if label_nodes.has(stat_name) and label_nodes[stat_name]:
				label_nodes[stat_name].text = ""  # clear any old value
				label_nodes[stat_name].visible = false
