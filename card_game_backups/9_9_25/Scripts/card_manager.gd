...
const MAX_REPULSION: float = 200.0      # increase from previous 30
const REPULSION_LERP_BASE: float = 0.2  # base lerp factor
...
var last_hovered_stack: Array = []
var active_jobs: Array = []  # each job = { "stack": stack_ref, "recipe": recipe_ref, "time_left": float }
var cards_moving: Dictionary = {}  # card -> target_position
var cards_to_free: Array = []
...
func merge_stacks(target_stack: Array, dragged_stack: Array) -> void:
	...
	for i in range(target_stack.size()):
		if is_instance_valid(target_stack[i]):
...
func mark_card_for_removal(card: Card) -> void:
	if is_instance_valid(card) and not cards_to_free.has(card):
		cards_to_free.append(card)

func free_marked_cards() -> void:
	for card in cards_to_free:
		cached_rects.erase(card)
	...
	for stack in all_stacks:
		...
		if intersection.size.x > 0 and intersection.size.y > 0:
			...
			if overlap_percent >= min_overlap_percent:
				overlapping_cards.append(top_card)
	return overlapping_cards

func get_card_global_rect(card: Node2D) -> Rect2:
	if not is_instance_valid(card):
		return Rect2()
	# Lazy update to prevent stale cached rects
	cached_rects[card] = calculate_card_global_rect(card)
...
#  CRAFTING FUNCTIONS
# ==============================
func find_matching_recipe(stack: Array) -> Dictionary:
	for recipe in RecipeDatabase.recipes:
		# Check persistent cards exist somewhere in stack
		var persistent_ok = true
		for p in recipe["persistent"]:
			if not stack.any(func(c): return c.subtype == p):
				persistent_ok = false
				break
		if not persistent_ok:
			continue
		# Check consumed cards (must be contiguous at top of stack)
		var consumed = recipe["consumed"]
		if consumed.size() > stack.size():
			continue
		var top_slice = stack.slice(stack.size() - consumed.size(), stack.size())
		var top_subtypes = []
		for c in top_slice:
			top_subtypes.append(c.subtype)
		if top_subtypes != consumed:
			continue
		# If both checks pass → valid recipe
		return recipe
	return {}

func check_and_start_jobs():
	for stack in all_stacks:
		var recipe = find_matching_recipe(stack)
		if recipe.is_empty():
			continue
		# Start new job if not already running on this stack
		if not active_jobs.any(func(j): return j["stack"] == stack):
			active_jobs.append({
				"stack": stack,
				"recipe": recipe,
				"time_left": recipe["craft_time"]
			})
			print("Started crafting job:", recipe)

func handle_crafting(delta: float) -> void:
...
func complete_job(job: Dictionary) -> void:
	...
	for subtype in recipe["consumed"]:
		if stack.size() == 0:
			break
		var card = stack.pop_back()
		# Only mark for removal if it’s not being dragged
		if is_instance_valid(card) and not card.is_being_dragged:
			mark_card_for_removal(card)
	# --- Spawn output cards at stack base ---
	var base_pos = Vector2(400, 300)
	if stack.size() > 0 and is_instance_valid(stack[0]):
		base_pos = stack[0].position
	for subtype in recipe["output"]:
		var new_card = spawn_card(subtype, base_pos)
		stack.append(new_card)
	# --- Clean invalid cards ---
	var idx = all_stacks.find(stack)
	if idx != -1:
		all_stacks[idx] = stack.filter(func(c): return is_instance_valid(c))
	if stack.size() > 0:
		update_stack_visuals(stack, stack[0].position)
	print("Crafting complete! Produced:", recipe["output"])
